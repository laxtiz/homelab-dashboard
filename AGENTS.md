...
Homelab monitoring dashboard: single Go binary with the Vue frontend embedded via `go:embed`. All commands go through `task` (see `Taskfile.yml`); don't run raw go/npm builds directly.

## Commands

- `task dev` — backend hot-dev: `go run ./cmd/dashboard -dev` (serves frontend from `web/dist` on disk)
- `task dev:web` — vite dev server on :5173 proxying `/api` + `/ws` to :8080; run in a separate terminal alongside `task dev`
- `task build` — builds frontend (runs `npm install`), then `go build -o bin/dashboard ./cmd/dashboard`
- `task test` — `go vet ./...` + `go test ./...` + `vue-tsc --noEmit` (frontend typecheck); depends on `web:build`
- `task lint` — gofmt check only: `test -z "$(gofmt -l cmd internal)"` (does not cover web/)
- `task run` — builds then runs the binary from the repo root

Full verification is `task lint && task test`.

## Gotchas

- `web/embed.go` does `//go:embed dist`, so **any** `go build`/`go test` fails if `web/dist` is missing. Always build the frontend first (`task build` or `task web:build`); CI fixed this ordering on purpose (see commit 9ed8f3b).
...
- Comments, README, Taskfile descriptions, and config comments are written in **Chinese** — match this convention.
...
- Container metrics connect to any Docker Engine API-compatible socket (docker, podman compat): resolution is `container.endpoint` config > `$DOCKER_HOST` > `unix:///var/run/docker.sock`. Docker socket isn't available in CI or bare `task test`.
